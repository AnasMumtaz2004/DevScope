  ...
  <div>
    <div className="grid gap-0.75" style={{ gridTemplateColumns: 'repeat(26, 1fr)' }}>
      {Array.from({ length: 182 }).map((_, i) => (
        <div
          key={i}
          className="aspect-square rounded-sm bg-zinc-200 dark:bg-zinc-700"
        />
      ))}
    </div>
    <Legend />
    ...
    <div className="relative">
      <div className="grid gap-0.75" style={{ gridTemplateColumns: 'repeat(26, 1fr)' }}>
        {days.map(({ key, level, count }) => (
          <div key={key} className="aspect-square rounded-sm cursor-default" style={{background: level === 0 && isDark ? EMPTY_DARK : COLORS[level],}} onMouseEnter={() => setTooltip({ key, count })} onMouseLeave={() => setTooltip(null)}/>
        ))}
      </div>
      {tooltip && (
